...
export default function HelpSidebar() {
    ...
    return (
        ...
        <aside className="md:w-64 mb-16 md:mb-0 md:mr-10 md:shrink-0">
            <h4 className="text-lg font-semibold px-3 pb-3 border-b border-gray-4 text-slate-800 dark:text-slate-200">Choose a category</h4>
            <nav>
                <ul>
                    {links.map((link, linkIndex) => (
                        <li key={linkIndex} className="py-2">
                            <Link
                                className={`flex items-center px-3 py-2 text-base group transition duration-150 ease-in-out ${pathname === link.href ? 'text-blue-500' : ' text-slate-11 hover:text-indigo-500'}`}
                                href={link.href}>
                                <span>{link.name}</span>
                                <svg
