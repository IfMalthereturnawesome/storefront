...
const ProductChoice: React.FC<ProductChoiceProps> = ({product1, product2, currentProductTitle}) => {
    ...
    const renderButton = (productTitle: string) => {
        if (productTitle === currentProductTitle) {
            return <span
                className="block font-sans text-base font-normal tracking-normal text-zinc-500 leading-[1.28577] py-1 md:py-2">Currently viewing</span>
        } else {
            return (
                <Link href={`/products/${product2.handle}`}>
                    <SecondaryButton variant={"third"}
                                     className={"rounded-[1.5rem] capitalize group text-md !py-1 px-1 min-h-[1rem] md:!py-2 md:px-2 md:min-h-[1.5rem] "}>
                        Read more
                    </SecondaryButton>
                </Link>
            );
    ...
    const renderFeatures = (features: Feature[]) => {
        return features.map((feature, index) => (
            <div key={index}
                 className="text-white pb-6 xl:pb-8 mx-auto px-vw-2 md:px-vw-4 lg:px-vw-1 justify-between flex-grow flex-col flex-1">
                ...
                {feature.description &&
                    <p className="p-0 m-0 min-w-0 max-w-xs font-sans text-sm md:text-base font-normal text-slate-10 mx-0 !leading-6">{feature.description}</p>}
            </div>
        ));
    };
    ...
    const renderProductSection = (product: ProductData) => {
        ...
        return (
            <div
                className="flex flex-col justify-between max-w-full px-2 md:flex-row md:items-center md:justify-center">
                <div
                    className="flex relative flex-col items-center md:justify-between pb-8 md:pb-2 md:px-[1.0588em] z-10 ">
                    ...
                    {/* News */}
                    <div
                        className="flex space-x-2 mb-2 md:mb-1 items-center justify-center h-8 rounded-full text-red-1 dark:text-red-11 bg-red-10 dark:bg-red-6 border-red-6 hover:bg-red-7 hover:bg-opacity-40 hover:text-red-1 disabled:hover:bg-red-7 disabled:hover:text-red-1 dark:bg-opacity-20 bg-opacity-20 text-xs font-semibold w-auto px-4"
                    >
                        <span>{product.news}</span>
                    </div>

                    <h3 className="header-bg-clip my-0 md:my-1 text-xl xs:text-2xl font-sans hidden dark:block min-h-[3.5rem] max-h-[3.5rem] md:min-h-fit md:max-h-fit">{product.title}</h3>
                    <h3 className="my-0 md:my-4 font-bold header-bg-clip-light text-xl xs:text-2xl font-sans dark:hidden min-h-[3.5rem] max-h-[3.5rem]">{product.title}</h3>
                    ...
                    <div
                        className="flex flex-wrap gap-4 justify-center items-center my-6 md:my-2 max-w-full min-h-[3.5rem] max-h-[3.5rem] ">
    ...
    return (
        <section
            className="relative pt-0 pb-12  md:pt-8 lg:pt-10 xl:pt-12 2xl:pt-16 3xl:pt-20 md:pb-20 tracking-tight leading-6 text-neutral-100 bg-gradient-to-r from-cyan-2 to-cyan-1 dark:from-cyan-3 dark:to-cyan-1 "
        >
            <div
                className="leading-6 bg-black  md:bg-transparent mx-auto px-0 sm:px-4 md:px-0 max-w-full md:max-w-[80vw] lg:max-w-[68vw]">
                <div className="text-left mx-auto">
                    <div className="md:bg-transparent content-start pt-10 md:pt-0 pb-5 md:pb-10  md:items-baseline">
                ...
                <div className="md:rounded-[2.5rem] bg-black md:shadow shadow-amberA-10 dark:shadow-amberA-12">
                    <div className="text-center">
                        <div
                            className="mx-auto max-w-[100vw] md:max-w-[80vw] lg:max-w-[65vw] xl:max-w-[55vw] 2xl:max-w-[40vw]">

                            {/* Product Sections */}
                            <div
                                className="flex  flex-row justify-center pt-8 md:pt-24 xl:pt-24 text-neutral-100">
                                {products.map((product, index) => (
                                    <div key={product.title}
                                         className="flex flex-col flex-1 space-y-6 md:space-y-0 w-full md:w-1/2">
                            ...
                            <div className="flex flex-row gap-0  justify-center pb-vw-10">
                                {products.map(product => (
                                    <div key={product.title + "-features"}
                                         className="flex flex-col w-1/2 px-1 md:px-2 justify-between ">
                                        {renderFeatures(product.features)}
                                    </div>
                                ))}
                            </div>
